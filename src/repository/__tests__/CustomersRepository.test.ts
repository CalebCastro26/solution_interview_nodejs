...
describe('CustomersRepositoryImpl', () => {
  describe('findByFilter', () => {
    it('should return empty array if there is not results', async () => {
      // Prepare
      const emptyResults: Customer[] = [];
      mockedAxios.get.mockResolvedValue(emptyResults);
      ...
      // Validate
      expect(response).toEqual(emptyResults);
    });
  });
});
